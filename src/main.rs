...
fn main() {
    unsafe {
        ...
        ).expect("Unable to find suitable memory type for index buffer");
        ...
        let index_ptr: *mut c_void = base.device.map_memory(index_buffer_memory, 0, index_buffer_memory_req.size, vk::MemoryMapFlags::empty()).unwrap();
        ...
        let vertex_input_buffer_memory_index = find_memory_type_index(
            ...
            &base.device_memory_properties,
            vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT
        ).expect("Unabe to find suitable memory type for vertex buffer");
        ...
        let uniform_color_buffer = base.device.create_buffer(&uniform_color_buffer_info, None).unwrap();
        let uniform_color_buffer_memory_req = base.device.get_buffer_memory_requirements(uniform_color_buffer);
        ...
        let uniform_color_buffer_memory = base.device.allocate_memory(&uniform_color_buffer_allocate_info, None).unwrap();
        ...
        let image_buffer_memory = base.device.allocate_memory(
            &image_buffer_allocate_info,
            None
        ).unwrap();
